...
public class RsaValidationTests {
    ...
    @Test
    public void rsaKeyValidation() throws Exception {
        for(String keyStr : keys.values())
        {
            ...
            KeyPair keyPair = key.toRSA(true);
    ...
    public void rsaPrivateKeyValidation() throws Exception {
        for(String keyStr : keys.values())
        {
            ...
            encryptDecrypt(publicKey, keyPairWithPrivate.getPrivate());
        }
    }
    
    private void encryptDecrypt(PublicKey publicKey, PrivateKey privateKey) throws Exception {
    ...
    private byte[] encrypt(PublicKey key, byte[] plaintext) throws Exception {
        Cipher cipher = Cipher.getInstance(TRANSFORMATION);   
        cipher.init(Cipher.ENCRYPT_MODE, key);  
        return cipher.doFinal(plaintext);
    }

    private byte[] decrypt(PrivateKey key, byte[] ciphertext) throws Exception {
        Cipher cipher = Cipher.getInstance(TRANSFORMATION);   
        cipher.init(Cipher.DECRYPT_MODE, key);  
        return cipher.doFinal(ciphertext);
    }
    
    private void validateRsaKey(KeyPair keyPair, JsonWebKey key) throws Exception {
